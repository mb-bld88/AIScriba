...
const LoadingOverlay: React.FC<{ message: string }> = ({ message }) => (
    <div className="absolute inset-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm flex flex-col items-center justify-center z-50 rounded-lg">
        ...
        try {
            ...
            if (meetingType === 'browser-tab') {
                try {
                    ...
                    // CHECK CRITICO: L'utente ha condiviso l'audio?
                    ...
                    // Stop recording if user clicks "Stop sharing" on browser UI
            ...
            mediaRecorderRef.current = new MediaRecorder(finalStreamForRecorder, { mimeType: 'audio/webm', audioBitsPerSecond: 64000 });
...
export const RecordingPage: React.FC<RecordingPageProps> = ({ onCancel, onAddMeeting, logUsage, currentUser, meetingDetails }) => {
  ...
  const handleSubmit = async () => {
    ...
    // Check for API Key
    if (!currentUser.googleApiKey && !isDemoMode) {
        setError("Chiave API mancante. Vai nelle Impostazioni (in alto a destra) e inserisci la tua Google Gemini API Key personale.");
        return;
    }

    if (!currentUser.companyId) { setError("Errore interno: Utente senza azienda."); return; }
    ...
    try {
        ...
        if (isDemoMode) {
            ...
            verbal = { executiveSummary: "Mock...", decisions: [], actionItems: [], discussionSummary: "Mock...", fullTranscript: "Mock...", flowchart: 'graph TD; A-->B;' };
        } else {
            // Pass the user's API Key
            verbal = await generateMinutesFromAudio(audioBlob, meetingDetails.participants, language, currentUser.googleApiKey!);
        }
        ...
        const newMeeting: Meeting = {
            ...
            verbal: verbal,
            status: 'processed',
            audioUrl: base64Audio, // Save persistent base64 string
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <div className="relative bg-white dark:bg-slate-800 rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8">
        {isLoading && <LoadingOverlay message={t('generateMinutes')} />}
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
          <div className="flex justify-end items-center">
            <button onClick={handleSubmit} disabled={isLoading || !audioBlob} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-lg flex items-center disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed transition-all duration-300 text-lg w-full justify-center">
              {isLoading ? <Spinner /> : <><i className="fa-solid fa-wand-magic-sparkles mr-2"></i>{t('generateMinutes')}</>}
            </button>
          </div>
        </div>
      </div>
